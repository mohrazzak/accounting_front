...
const index = () => {
  ...
  const dispatchers = {
    edit: (editedDaily) => {
      dispatch(
        editDaily({
          ...editedDaily,
          value: Number(parseFloat(editedDaily.value).toFixed(2)),
          values: Number(parseFloat(editedDaily.values).toFixed(2)),
  ...
  useEffect(() => {
    dispatch(getDaily({ all: true, userId: params.id, isDaily: false }));
  }, []);
      ...
      <Box>
        ...
        <Box>
          ...
          {/* <Button
            ...
            color="error"
            onClick={() => navigate('invoices/add', { state: { t: 't' } })}
          >
            {`انشاء فاتورة`}
          </Button> */}
        </Box>
